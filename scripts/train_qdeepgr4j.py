...
def get_dataloaders(train_ds, val_ds, gr4j_run_dir, station_id, batch_size, **kwargs):
    ...
    process_features = prod_store(X, include_x=True)[0]
    inp_train = process_features[:train_len]
    inp_val = process_features[train_len:]
...
def convert_results_to_df(rmse_score, nse_score, nnse_score, confidence_score, quantiles, forecast_horizon):

    # Convert to dataframe
    res = np.concatenate([rmse_score, nse_score, nnse_score], axis=0)
    index = pd.MultiIndex.from_product([['RMSE', 'NSE', 'Normalized NSE'], quantiles], 
...
def train_and_evaluate(train_ds, val_ds,
    ...
    rmse_train = evaluation_metrics[:, 0]
    nse_train = evaluation_metrics[:, 1]
    nnse_train = evaluation_metrics[:, 2]

    # Convert results to dataframe
    res_train = convert_results_to_df(rmse_train, nse_train, nnse_train, 
    ...
    # Extract evaluation metrics
    rmse_val = evaluation_metrics[:, 0]
    nse_val = evaluation_metrics[:, 1]
    nnse_val = evaluation_metrics[:, 2]

    # Convert results to dataframe
    res_val = convert_results_to_df(rmse_val, nse_val, nnse_val,
...
if __name__ == '__main__':
    ...
    # Save results to csv
    all_res.reset_index().to_csv(os.path.join(args.run_dir, 'results.csv'))
    avg_res = all_res.T.mean().reset_index()
    avg_res = pd.pivot_table(avg_res, index=['Quantile', 'Dataset'], columns='Metric').reset_index()

    avg_res.reset_index(drop=True, inplace=True)
    avg_res.columns = ['Quantile', 'Dataset', 'NSE', 'Normalized NSE', 'RMSE']

    print(avg_res)
    avg_res.to_csv(os.path.join(args.run_dir, 'average_results.csv'))
