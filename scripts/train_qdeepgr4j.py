...
from model.ml import ConvNet, LSTM
...
def evaluate_preds(model, prod_store, ds, batch_size, y_mu, y_sigma, q_in, quantiles=[0.5]):
    ...
    return evaluate(P, ET, Q, Q_hat, quantiles=quantiles), out_of_interval(targets=Q, predictions=Q_hat)
...
def train_and_evaluate(train_ds, val_ds,
    ...
    for epoch in pbar:
        ...
        # Train step
        train_loss = train_step(model, train_dl, kwargs['quantiles'], opt)

        # Validation step
        val_loss = val_step(model, val_dl, kwargs['quantiles'])
